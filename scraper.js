...
async function getTableIdx(header, properties) {
  ...
  const tableIdx = await parseJsonCodeBlock(
...
async function scrapeTable(
  ...
  const tableHeader = await page.evaluate(() => {
    return Array.from(document.querySelectorAll("table thead tr")).map(
      (row) => {
        return Array.from(row.querySelectorAll("th")).map((cell) =>
          cell.innerText.trim()
        );
      }
    );
  ...
  while (true) {
    ...
    if (currentPageIdx < startPageIdx) {
      ...
      (endPageIdx === "all" || currentPageIdx <= endPageIdx)
    ) {
    ...
    await page
      ...
      .catch(() => false); // Catch timeout errors to avoid crashing
  }
